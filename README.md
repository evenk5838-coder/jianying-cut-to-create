...
中文非官方产品概念网站。在原项目上改版为 **滚动控制的全屏作品叙事**：地球 → 雾林 → 时尚人物 → 原声口播 → 城市 → 未来 → 完整短片。

[公开源码](https://github.com/evenk5838-coder/jianying-cut-to-create) · [正式网站](https://jianying-cut-to-create.evenk5838.chatgpt.site)

React 19 + TypeScript + Vite + GSAP ScrollTrigger + 原生 CSS。旧的 Three.js / R3F 穿梭空间、模拟编辑器和编号导航已移除。无需后端或 AI API 密钥。
...
`dist/` 是可部署的静态网站。首屏只挂载地球媒体源；其他片段在接近时加载。桌面影片以 1080p 为主；轻量版是专门裁切的 720×1280。当前和相邻片段最多两个媒体源，离屏暂停并释放远处源。滚动期间保持当前帧，停留 140 毫秒后继续单次播放；原声口播在本幕内保持同期声音。手机默认轻量版，可在页脚切换。
